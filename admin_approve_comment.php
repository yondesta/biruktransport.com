...
if (isset($_GET["id"]) && !empty(trim($_GET["id"]))) {
    ...
    $sql = "UPDATE event_comments SET is_approved = TRUE WHERE id = ?";

    if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param("i", $comment_id);

        if ($stmt->execute()) {
            $_SESSION['comment_action_success'] = "Comment approved successfully!";
        } else {
            $_SESSION['comment_action_error'] = "ERROR: Could not approve comment. " . $stmt->error;
        }
        $stmt->close();
    } else {
